...
export class QuestionsService {
  ...
  async findAll(query: QueryQuestionDto, taoBoi?: number) {
    ...
    if (taoBoi !== undefined) qb.andWhere('ch.taoBoi = :taoBoi', { taoBoi });
    if (search) qb.andWhere('ch.noiDung LIKE :s', { s: `%${search.trim()}%` });
    if (maMonHoc !== undefined)
      qb.andWhere('ch.maMonHoc = :maMonHoc', { maMonHoc });
    if (doKho !== undefined) qb.andWhere('ch.doKho = :doKho', { doKho });
  ...
  private async taoTrongTransaction(
    ...
    await this.kiemTraTrungNoiDung(
      dto.noiDung,
      dto.maMonHoc,
      taoBoi,
      undefined,
    ...
    if (dto.loaiCauHoi === LoaiCauHoi.MOT_DAP_AN && dung.length !== 1)
      throw new BadRequestException(
        'Câu hỏi 1 đáp án phải có đúng 1 đáp án đúng',
      );
    if (dto.loaiCauHoi === LoaiCauHoi.NHIEU_DAP_AN && dung.length < 2)
      throw new BadRequestException(
        'Câu hỏi nhiều đáp án phải có ít nhất 2 đáp án đúng',
      );

    const cauHoi = await em.save(
      CauHoi,
      em.create(CauHoi, {
        noiDung: dto.noiDung,
  ...
  async update(id: number, dto: UpdateQuestionDto, taoBoi?: number) {
    ...
    // Dùng chủ sở hữu của câu hỏi, không dùng tham số taoBoi (undefined với Admin).
    await this.kiemTraTrungNoiDung(
      noiDungMoi,
      maMonHocMoi,
      cauHoiCu.taoBoi,
      ...
      if (dto.luaChons) {
        ...
        const saved = await Promise.all(
          dto.luaChons.map((lc) =>
            em.save(
              LuaChon,
              em.create(LuaChon, { maCauHoi: id, noiDung: lc.noiDung }),
            ),
          ),
        );
  ...
  // Chặn trùng nội dung câu hỏi trong NGÂN HÀNG RIÊNG của người tạo, theo từng môn học.
  // Câu hỏi thuộc về GV (mọi đường đọc đều lọc taoBoi), nên không so với câu của GV khác.
  ...
  private async kiemTraTrungNoiDung(
    ...
    const qb = repo
      .createQueryBuilder('ch')
      .where('ch.taoBoi = :taoBoi', { taoBoi })
      .andWhere('ch.maMonHoc = :maMonHoc', { maMonHoc })
      .andWhere('ch.noiDung = :noiDung', { noiDung });
    if (boQuaId) qb.andWhere('ch.maCauHoi != :boQuaId', { boQuaId });
    if (await qb.getCount())
      throw new BadRequestException(
        'Bạn đã có câu hỏi với nội dung này trong môn học này',
      );
  }
}
